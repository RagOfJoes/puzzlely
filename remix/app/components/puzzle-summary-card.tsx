...
>(({ className, isEditable, puzzle, ...props }, ref) => (
	<Primitive.div
		{...props}
		className={cn(
			"h-full w-full border bg-muted p-4",
	...
	>
		<div className="flex justify-between gap-2">
			<div className="flex w-full min-w-0 items-center gap-2">
				...
				>
					<p className="w-full truncate text-sm font-bold leading-none text-current">
						{puzzle.difficulty}
					</p>
				</div>
				...
				>
					<p className="truncate text-lg font-bold leading-none">{puzzle.created_by.username}</p>
				</Link>
			</div>
		...
		<time
			...
			className="mt-4 line-clamp-1 w-full text-ellipsis text-xs font-semibold tracking-wide text-muted-foreground"
		>
