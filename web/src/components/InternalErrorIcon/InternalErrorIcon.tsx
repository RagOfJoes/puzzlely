...
const InternalErrorIcon = (props: IconProps) => {
  const { colors } = useTheme();

  const wires = '#ccc';
  //  Leaves
  const leftBackLeaf = colors.text.primary;
  const leftFrontLeaf = colors.primary;
  const rightBackLeaf = colors.primary;
  const rightFrontLeaf = colors.text.primary;
  const circleBackground = colors.surface;
  //  Server rack
  const serverRackLight = colors.primary;
  const serverRackPC = colors.background;
  const serverRackSide = colors.secondary;
  const serverRackSurface = colors.surface;
  // Monitor
  const monitorScreen = colors.surface;
  const monitorBezel = colors.text.primary;
  const monitorStand = colors.text.primary;
  const monitorBottomBezel = colors.text.primary;
  // Peripherals
  const mouse = colors.text.primary;
  const keyboard = colors.text.primary;
  // Floors
  const floors = colors.surface;
  // Beings
  const beingHat = colors.primary;
  const beingColor = colors.slate;

  return (
    <Icon viewBox="0 0 1119.60911 699" {...props}>
      <circle cx="292.60911" cy="213" r="213" fill={circleBackground} />
      <path
        fill={leftBackLeaf}
      ...
      />
      <ellipse rx="187" cy="424.5" ry="25.43993" cx="198.60911" fill={floors} />
      <ellipse cx="198.60911" cy="424.5" rx="157" ry="21.35866" opacity="0.1" />
      <ellipse cx="836.60911" cy="660.5" rx="283" ry="38.5" fill={floors} />
      <ellipse cx="310.60911" cy="645.5" rx="170" ry="23.12721" fill={floors} />
      <path
        fill="none"
        stroke={wires}
        strokeWidth="2"
      ...
      <rect
        ...
        height="242"
        x="108.60911"
        fill={serverRackSurface}
      />
      <rect x="87.60911" y="135" width="250" height="86" fill={serverRackPC} />
      <rect x="87.60911" y="237" width="250" height="86" fill={serverRackPC} />
      <rect x="87.60911" y="339" width="250" height="86" fill={serverRackPC} />
      <rect
        y="150"
        width="16"
        height="16"
      ...
      <rect
        ...
        fill={serverRackLight}
      />
      <circle cx="316.60911" cy="538" r="79" fill={beingColor} />
      <rect x="280.60911" y="600" width="24" height="43" fill={beingColor} />
      <rect x="328.60911" y="600" width="24" height="43" fill={beingColor} />
      <ellipse cx="300.60911" cy="643.5" rx="20" ry="7.5" fill={beingColor} />
      <ellipse cx="348.60911" cy="642.5" rx="20" ry="7.5" fill={beingColor} />
