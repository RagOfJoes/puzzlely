...
const githubCallback = async (
  req: NextApiRequest & {
    github?: { accessToken?: string; profile?: Profile };
  },
  res: NextApiResponse
) => {
  try {
    ...
    await middleware(
      ...
      passport.authenticate('github', {
        session: false,
        failureRedirect: '/',
        assignProperty: 'github',
      })
    );

    const r = await fetch(`${process.env.API_URL}/auth/github`, {
      method: 'POST',
      credentials: 'include',
      headers: {
        // This is from passport
        Authorization: `Bearer ${req?.github?.accessToken}`,
