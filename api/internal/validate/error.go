...
var (
	tagTypes map[string][]string = map[string][]string{
		// Format tags
		"format": {
			"cidr", "cidrv4", "cidrv6", "datauri", "fqdn", "hostname",
			"hostname_port", "hostname_rfc1123", "tcp_addr", "tcp4_addr",
			"tcp6_addr",
			"udp_addr", "udp4_addr", "udp6_addr", "ip_addr", "ip4_addr", "ip6_addr",
			"ip", "ipv4", "ipv6", "unix_addr", "mac", "uri", "url", "url_encoded",
			"urn_rfc2141", "alpha", "alphanum", "alphanumspace", "alphaunicode",
			"alphanumunicode", "ascii", "lowercase", "multibyte", "number", "numeric",
			"printascii", "printasciiextra", "uppercase", "base64", "base64url",
			"btc_addr", "btc_addr_bech32", "datetime", "e164", "email", "eth_addr",
			"hexadecimal", "hexcolor", "hsl", "hsla", "html", "html_encoded", "isbn",
			"isbn10", "isbn13", "json", "latitude", "longitude", "rgb", "rgba",
			"ssn", "uuid", "uuid_rfc4122", "uuid3", "uuid3_rfc4122", "uuid4",
			"uuid4_rfc4122", "uuid5", "uuid5_rfc4122", "dir", "file", "isdefault",
			"unique",
		},
		// Comparison tags
		"comparison": {
			"eqcsfield", "eqfield", "containsfield", "excludesfield", "gtcsfield",
			"gtecsfield", "gtefield", "gtfield", "ltcsfield", "ltecsfield",
			"ltefield", "ltfield", "necsfield", "nefield", "eq", "gt",
			"gte", "lt", "lte", "ne", "len", "min", "max", "oneof", "excludes",
			"excludesall", "excludesrune", "contains", "containsany", "containsrune",
			"startswith", "endswith",
		},
	}
	...
	templates map[string]string = map[string]string{
		...
		"required_if":          "must not be blank",
		"required_unless":      "must not be blank",
		"required_with":        "must not be blank",
		"required_with_all":    "must not be blank",
		"required_without":     "must not be blank",
		"required_without_all": "must not be blank",
		"unique":               "must contain only unique values",
		// Custom
		//
		"notblank":      "must not be blank",
		"alphanumspace": "must only contain letters, numbers, '-', '_', and spaces",
	}
)
	...
	for k, v := range templates {
		if k == e.TagName {
			...
			if code != "validation" {
				...
				case "comparison":
					if e.TagName == "oneof" {
						return fmt.Sprintf("%s %s %s.", e.Field, template, strings.Join(strings.Split(e.TagParam, " "), ", "))
					}
